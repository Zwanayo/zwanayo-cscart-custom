...
<div class="ut2-pb ty-product-block ty-product-detail ut2-three-columns {if $settings.abt__ut2.products.view.show_sticky_add_to_cart[$settings.abt__device] == 'Y' && !in_array($product.zero_price_action, ["P","A"]) && $product.price > 0 && $product.amount > 0 && $is_add_to_cart_mv} sticky_add_to_cart{/if} {if $settings.abt__ut2.products[$settings.abt__details_layout].multiple_product_images[$settings.abt__device] > 1}{if $product_images_count == 1}--double{elseif $product_images_count < 1}--single{/if}{/if}">
