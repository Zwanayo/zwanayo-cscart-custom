...
{foreach from=$splited_elements item="row"}
    ...
    <div class="ty-menu__submenu-col" {if $settings.abt__device !== "mobile"}style="{if $item1.abt__ut2_mwi__dropdown == "YesNo::NO"|enum}width:{$col_width}%;{else}height: var(--ut2-vertical-menu-block-height);{/if}"{/if}>
        {foreach from=$row item="item2" name="item2"}
            <div class="second-lvl" data-elem-index="{$second_level_counter}">
                ...
                <div class="ty-menu__submenu-item-header{if $item2.abt__ut2_mwi__icon} ut2-mwi-icon-wrap{/if}{if empty($item2.$childs)} no-items{/if}">
                    <a {if $settings.abt__device === "desktop" || !$item2.childs}href="{$item2_url|default:"javascript:void(0)"}"{else}href="javascript:void(0)"{/if}{if $item2_url && $item1.new_window === "YesNo::YES"|enum} target="_blank"{/if} class="ty-menu__submenu-link {if $item2.class}{$item2.class}{/if}">
                        {if $item2.abt__ut2_mwi__icon && $block.properties.abt_menu_icon_items === "YesNo::YES"|enum && $settings.abt__device === "desktop"}
                ...
                {if !empty($item2.$childs)}
                    ...
                    <div class="ty-menu__submenu {if $item1.abt__ut2_mwi__dropdown === "YesNo::YES"|enum}tree-level {/if}{if $item2.abt__ut2_mwi__text && $item2.abt__ut2_mwi__text_position !== "bottom"}tree-level-img {/if}" {if $item1.abt__ut2_mwi__dropdown =="YesNo::YES"|enum && $settings.abt__device !== "mobile"}style="height: var(--ut2-horizontal-menu-block-height)"{/if}>
                        {if $item1.abt__ut2_mwi__dropdown === "YesNo::YES"|enum}
                            ...
                            {foreach from=array_chunk($item2.$childs, ceil($item2.$childs|count / $cols_tree_level), true) item="item2_childs"}
                                        ...
                                        {foreach from=$item2_childs item="item3" name="item3"}
                                            ...
                                            <div class="ty-menu__submenu-item">
                                                <a href="{$item3_url|default:"javascript:void(0)"}"{if $item3_url && $item1.new_window === "YesNo::YES"|enum} target="_blank"{/if} class="ty-menu__submenu-link{if $item3.class} {$item3.class}{/if}" title="{$item3.$name nofilter}">
                                                    {if $block.properties.abt_menu_icon_items === "YesNo::YES"|enum && $item3.abt__ut2_mwi__icon && $settings.abt__device === "desktop"}
                                ...
                                {/if}
                            {/foreach}
                            {if $item1.abt__ut2_mwi__dropdown === "YesNo::YES"|enum && $item2.abt__ut2_mwi__text && $item2.abt__ut2_mwi__text_position !== "bottom" && $settings.abt__device !== "mobile"}
                        ...
                        {else}
                            ...
                            {/if}
                        {/if}
                        {if $item1.abt__ut2_mwi__dropdown === "YesNo::YES"|enum && $item2.abt__ut2_mwi__text && $settings.abt__device !== "mobile"}
                            <div class="ut2-mwi-html {$item2.abt__ut2_mwi__text_position} hidden-phone" style="height: var(--ut2-horizontal-menu-block-height)">{$item2.abt__ut2_mwi__text nofilter}</div>
                        {/if}
                        {if $item2.$childs|count > $Viewlimit && $item1.abt__ut2_mwi__dropdown === "YesNo::NO"|enum}
                            <a href="{if $block.properties.abt__ut2_view_more_btn_behavior|default:'view_items' === 'view_items'}javascript:void(0);" onMouseOver="$(this).prev().addClass('view');$(this).addClass('hidden');{else}{$item2_url|default:"javascript:void(0)"}" rel="nofollow{/if}" class="ut2-more" title=""><span>{__("more")}</span></a>
                        {/if}
                        {if $item1.abt__ut2_mwi__dropdown === "YesNo::YES"|enum && $item2_url && $item2.show_more && $settings.abt__device === "desktop"}
